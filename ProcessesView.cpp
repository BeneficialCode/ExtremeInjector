...
ProcessesView::ProcessesView(TabManager& tm) :_tm(tm), _pm(Globals::Get().ProcMgr) {
	...
	g_pd3dDevice->CreateShaderResourceView(spTexture, &srvDesc, &m_spImage);
}
...
void ProcessesView::DoSort(int col, bool asc){
	std::sort(_processes.begin(), _processes.end(), [=](const auto& p1, const auto& p2) {
		...
		case 2: return SortHelper::SortNumbers(p1->Id, p2->Id, asc);
		case 3: return SortHelper::SortStrings(GetProcessInfoEx(p1.get()).UserName(), GetProcessInfoEx(p2.get()).UserName(), asc);
		case 4: return SortHelper::SortNumbers(p1->SessionId, p2->SessionId, asc);
		case 5: return SortHelper::SortNumbers(p1->CPU, p2->CPU, asc);
		case 6: return SortHelper::SortNumbers(p1->ParentId, p2->ParentId, asc);
		case 7: return SortHelper::SortNumbers(p1->CreateTime, p2->CreateTime, asc);
		case 8: return SortHelper::SortNumbers(p1->PrivatePageCount, p2->PrivatePageCount, asc);
		case 9: return SortHelper::SortNumbers(p1->BasePriority, p2->BasePriority, asc);
		case 10: return SortHelper::SortNumbers(p1->ThreadCount, p2->ThreadCount, asc);
		case 11: return SortHelper::SortNumbers(p1->HandleCount, p2->HandleCount, asc);
		case 12: return SortHelper::SortNumbers(p1->WorkingSetSize, p2->WorkingSetSize, asc);
		case 13: return SortHelper::SortStrings(GetProcessInfoEx(p1.get()).GetExecutablePath(), GetProcessInfoEx(p2.get()).GetExecutablePath(), asc);
		case 14: return SortHelper::SortNumbers(p1->KernelTime + p1->UserTime, p2->KernelTime + p2->UserTime, asc);
		case 15: return SortHelper::SortNumbers(p1->PeakThreads, p2->PeakThreads, asc);
		case 16: return SortHelper::SortNumbers(p1->VirtualSize, p2->VirtualSize, asc);
		case 17: return SortHelper::SortNumbers(p1->PeakWorkingSetSize, p2->PeakWorkingSetSize, asc);
		case 18: return SortHelper::SortNumbers(GetProcessInfoEx(p1.get()).GetAttributes(_pm), GetProcessInfoEx(p2.get()).GetAttributes(_pm), asc);
		case 19: return SortHelper::SortNumbers(p1->PagedPoolUsage, p2->PagedPoolUsage, asc);
...
void ProcessesView::BuildPriorityClassMenu(WinSys::ProcessInfo* pi){
	...
	if (BeginMenu("Priority")) {
		...
		if (MenuItem("Below Normal (6)", nullptr, pc == ProcessPriorityClass::BelowNormal,
			enabled && pc != ProcessPriorityClass::BelowNormal)) {
		...
		if (MenuItem("Above Normal (10)", nullptr, pc == ProcessPriorityClass::AboveNormal, enabled &&
			pc != ProcessPriorityClass::AboveNormal)) {
		...
		if (MenuItem("Realtime (24)", nullptr, pc == ProcessPriorityClass::Realtime, enabled
			&& pc != ProcessPriorityClass::Realtime)) {
...
void ProcessesView::BuildProperitesWindow(ProcessProperties* props){
...
void ProcessesView::BuildTable() {
	...
	if (BeginTable("processes", 20, ImGuiTableFlags_BordersV * 0 | ImGuiTableFlags_Sortable | ImGuiTableFlags_BordersH |
		...
		TableSetupColumn("Bits", ImGuiTableColumnFlags_NoHide | ImGuiTableColumnFlags_NoReorder);
		...
		if (_injectFailed) {
			if(MessageBoxResult::StillOpen != SimpleMessageBox::ShowModal("Inject Process", "Failed to inject DLL!"))
				_injectFailed = false;
		}
		...
		while (clipper.Step()) {
			for (int j = clipper.DisplayStart; j < clipper.DisplayEnd; j++) {
				...
				if (!_modalOpen) {
					if (_selectedProcess != nullptr && IsKeyPressed(ImGuiKey_Delete) && GetIO().KeyShift)
						_modalOpen = true;
				...
				if (_infoOpen && _selectedProcess == p) {
					CStringA text;
					if (_injectFailed) {
						text.Format("Failed to inject DLL into process %u (%ws)", p->Id, p->GetImageName().c_str());
					}
					else {
						text.Format("Injected DLL into process %u (%ws)", p->Id, p->GetImageName().c_str());
					}
					auto result = SimpleMessageBox::ShowModal("Info", text, MessageBoxButtons::OkCancel);
					if (result != MessageBoxResult::StillOpen) {
						_infoOpen = false;
					}
				}
...
void ProcessesView::BuildViewMenu(){
	if (BeginMenu("View")) {
		if (BeginMenu("Update Interval")) {
			...
			Separator();
			if (MenuItem("Pause", nullptr, _updateInterval == 0))
				TogglePause();
			ImGui::EndMenu();
		}
		ImGui::EndMenu();
